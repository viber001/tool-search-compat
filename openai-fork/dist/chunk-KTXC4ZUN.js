...
// src/responses/openai-responses-language-model.ts
var CODEX_TOOL_SEARCH_COMPAT_NAME = "tool_search_compat";
function hasCodexToolSearchHistory(prompt) {
  return prompt.some((message) => {
    if (typeof message.content === "string") return false;
    return message.content.some(
      (part) => (part.type === "tool-call" || part.type === "tool-result") && (part.toolName === "tool_search" || part.toolName === CODEX_TOOL_SEARCH_COMPAT_NAME)
    );
  });
}
...
var OpenAIResponsesLanguageModel = class _OpenAIResponsesLanguageModel {
  ...
  async getArgs({
    ...
    const baseToolNameMapping = createToolNameMapping({
      tools,
      providerToolNames: {
    ...
    const hasNativeToolSearch = tools?.some(
      (tool) => tool.type === "provider" && tool.id === "openai.tool_search"
    ) ?? false;
    const hasCodexToolSearch = !hasNativeToolSearch && (tools?.some(
      (tool) => tool.type === "function" && tool.name === "tool_search"
    ) ?? false);
    const toolNameMapping = hasCodexToolSearch ? {
      toProviderToolName: (customToolName) => customToolName === "tool_search" ? CODEX_TOOL_SEARCH_COMPAT_NAME : baseToolNameMapping.toProviderToolName(customToolName),
      toCustomToolName: (providerToolName) => providerToolName === CODEX_TOOL_SEARCH_COMPAT_NAME ? "tool_search" : baseToolNameMapping.toCustomToolName(providerToolName)
    } : baseToolNameMapping;
    ...
    const store = hasCodexToolSearch || hasCodexToolSearchHistory(prompt) ? true : openaiOptions?.store;
    ...
    const shellToolEnvType = tools?.find(
  ...
  async doGenerate(options) {
    ...
    if (process.env.OPENCODE_TOOL_SEARCH_COMPAT_DEBUG === "1") {
      appendFileSync(
        "/tmp/opencode-tool-search-compat-debug.log",
        `${JSON.stringify({
          store: body.store,
          tools: body.tools,
          prompt: options.prompt.map((message) => ({
            ...
            content: typeof message.content === "string" ? message.content : message.content.map((part) => ({
              type: part.type,
              toolName: "toolName" in part ? part.toolName : void 0
            }))
          }))
        })}
`
