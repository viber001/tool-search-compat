...
- **store** _boolean_
  ...
  OpenCode registers a local no-op `tool_search` handler in
  `~/.config/opencode/plugins/tool-search-noop.ts`; its execution returns `[]`. The fork does not add
  that handler to the Responses request `tools` array. If a proxy nevertheless returns a client-side
  `tool_search_call`, the fork consumes it internally and returns an empty `tools: []` catalog.
