...
- **store** _boolean_
  ...
  The compatibility fork also avoids serializing assistant-message, reasoning, and tool-search item
  references across normal OpenCode turns. It replays content instead because the configured compatible
  endpoint does not reliably persist `msg_*`, `rs_*`, and `tsc_*` items. For hidden tool-search
  follow-ups, reasoning is reconstructed from `reasoning.encrypted_content` without the server-side
  reasoning item ID. Unknown provider-executed `tsc_*` items that cannot be reconstructed are omitted.
