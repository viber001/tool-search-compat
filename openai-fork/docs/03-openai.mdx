...
- **store** _boolean_
  ...
  The compatibility fork also avoids serializing assistant-message and reasoning item references
  across normal OpenCode turns. It replays content instead because the configured compatible endpoint
  does not reliably persist `msg_*` and `rs_*` items. For hidden tool-search follow-ups, reasoning is
  reconstructed from `reasoning.encrypted_content` without the server-side reasoning item ID.
