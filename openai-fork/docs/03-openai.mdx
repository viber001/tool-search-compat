...
- **store** _boolean_
  ...
  This local fork automatically adds the Codex client-side `tool_search` tool to every Responses request.
  To keep the response items available for the follow-up request, the fork sends `store: true` explicitly
  and the effective value remains `true` even when `store: false` is provided. The `store: false` examples
  in the upstream documentation therefore do not disable persistence in this fork.
