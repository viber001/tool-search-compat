...
- **store** _boolean_
  ...
  follow-up contains only the tool-search call and its empty output. Assistant output and the ordinary
  function call are held and then returned to OpenCode, which executes the function and supplies its
  ...
  from `reasoning.encrypted_content` without the server-side reasoning item ID. Recognized `tool_search`
  history keeps the stock item-reference behavior; only unknown provider-executed `tsc_*` items that lost
  the protocol fields needed for reconstruction are omitted.
