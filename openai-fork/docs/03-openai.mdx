...
- **store** _boolean_
  ...
  The fork does not modify OpenCode's tool registry or add a plugin. If a proxy returns a client-side
  `tool_search_call`, the provider consumes it internally, sends a matching `tool_search_output` with
  an empty `tools: []` catalog, and continues the Responses request for up to three total rounds.
  Client-side tool-search protocol items are omitted from the final result returned to OpenCode.
  Explicit `openai.tools.toolSearch()` provider tools remain supported.
