...
- **store** _boolean_
  ...
  If a response contains both a pending `tool_search_call` and an ordinary `function_call`, the hidden
  follow-up contains the canonicalized assistant output followed by the tool-search call and its empty
  output. The ordinary function call is held and then returned to OpenCode, which executes it and supplies
  its `function_call_output` on the next turn. Pure tool-search rounds also retain earlier assistant-visible
  output in the final provider result.
  ...
  `providerOptions['headroom-openai-fork']`, and validates it with the same complete Responses options
