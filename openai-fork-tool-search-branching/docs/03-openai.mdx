...
- **store** _boolean_
  ...
  This local fork sends `store: true` explicitly when the option is omitted because some compatible
  endpoints interpret an omitted value as `false`. An explicit `store: false` remains `false`.

  This is a fork-specific wire-format difference from the stock `@ai-sdk/openai` package: the stock
  package may omit `store` when it is not configured. Both packages can still point to the same proxy;
  changing the `npm` provider entry changes which SDK serializes the request.

  The fork does not modify OpenCode's tool registry or add a plugin. If a proxy returns a
  `tool_search_call` without a matching `tool_search_output` in the same response, the provider
  consumes it internally, sends an empty `tools: []` result, and continues the Responses request for
  up to three total rounds. This includes compatible endpoints that label an unresolved call as
  `execution: 'server'`. Internally completed tool-search protocol items are omitted from the final
  result returned to OpenCode. Server calls that already have an output and explicit
  `openai.tools.toolSearch()` provider tools remain supported.

  Assistant-visible message, reasoning, and compaction output is converted through the same
  `convertToOpenAIResponsesInput()` path used by normal OpenCode prompts before it is placed in a hidden
  follow-up. This avoids replaying raw Responses message objects whose JSON shape differs from the next
  OpenCode request and allows the stable prefix to remain eligible for prompt-cache reuse.

  This branching variant treats mixed output differently. If a response contains both a pending
  `tool_search_call` and an ordinary `function_call`, it filters the pending tool-search item and returns
  the assistant output plus the ordinary function call directly to OpenCode. No hidden
  `tool_search_call/tool_search_output` request is sent for that mixed response. OpenCode executes the
  function and its next canonical request contains the matching `function_call_output`. Pure pending
  tool-search responses still use the hidden follow-up and retain their earlier assistant-visible output.

  The fork reads the standard `providerOptions.openai` namespace first. When that namespace is absent,
  it falls back to the configured provider namespace, for example
  `providerOptions['headroom-openai-branching']`, and validates it with the same complete Responses options
  schema. This applies to `reasoningEffort`, `store`, `promptCacheKey`, `textVerbosity`, `include`, and
  the other supported OpenAI options.

  The compatibility fork also avoids serializing assistant-message and reasoning item references across
  normal OpenCode turns. It replays content instead because the configured compatible endpoint does not
  reliably persist `msg_*` and `rs_*` items. For hidden tool-search follow-ups, reasoning is reconstructed
  from `reasoning.encrypted_content` through the canonical prompt converter instead of an `rs_*`
  `item_reference`. Recognized `tool_search` history keeps the stock item-reference behavior; only unknown
  provider-executed `tsc_*` items that lost the protocol fields needed for reconstruction are omitted.

  Responses `doStream()` is buffered in this fork: it completes `doGenerate()` first, including any
  hidden tool-search rounds, and only then exposes an AI SDK stream. This differs from the stock native
  SSE path in time-to-first-output, chunking, and cancellation behavior.
